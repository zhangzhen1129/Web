...
const currencyText = (value, code) => `${CURRENCY_PREFIX}${stringValue(value, code).trim()}`
...
function summary({ availableText, totalText, usedText, locked, prefix = 'CASH' }) { const lockedValue = integerValue(locked, `${prefix}_LOCKED_INVALID`) !== 0; return { availableLabelText: 'Cantidad disponible', availableText: currencyText(availableText, `${prefix}_AMOUNT_INVALID`), totalLabelText: 'Crédito total', totalText: stringValue(totalText, `${prefix}_TOTAL_CREDIT_INVALID`), usedLabelText: 'Crédito usado', usedText: stringValue(usedText, `${prefix}_USED_CREDIT_INVALID`), locked: lockedValue, refreshEnabled: !lockedValue } }
