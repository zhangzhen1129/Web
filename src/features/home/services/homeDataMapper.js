...
function localSelection() { return { amountOptions: Array.from({ length: 50 }, (_, index) => { const amount = (index + 1) * 100; return { key: String(amount), text: String(amount), disabled: false } }), selectedAmountKey: '5000', termOptions: [91, 120, 180].map((term) => ({ key: String(term), text: `${term} días`, disabled: false })), selectedTermKey: '91' } }
