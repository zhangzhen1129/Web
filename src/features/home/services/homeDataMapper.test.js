...
test('maps exact decimal product totals and a UI-contract-valid multi-push model', () => { const mapped = mapMultiPushResponse(multi({ products: [{ id: 'p1', productName: 'Product', minAmount: '1.20', isReloan: 1, icon: 'https://example.com/i.png' }, { id: 'p2', productName: 'Product 2', minAmount: '2.30', isReloan: 0, icon: 'https://example.com/j.png' }] }), { requestId: 'cycle-2', revision: 2, now: new Date(2026, 0, 1).getTime(), random: () => 0 }); assert.equal(mapped.payload.multiPushViewData.creditSummary.availableText, '3.5'); assert.equal(mapped.payload.multiPushViewData.creditSummary.totalText, '100'); assert.equal(mapped.payload.multiPushViewData.creditSummary.usedText, '10'); assert.equal(mapped.payload.multiPushViewData.products[0].loanAmountText, '1.20'); assert.equal(mapped.payload.multiPushViewData.products[0].dueDateText, '2026-01-07'); assert.deepEqual(validateHomeViewPayload(mapped.payload), []) })
