...
test('maps a non-2000 business response to a loading skeleton with its safe toast', async () => { const provider = createHomeDataProvider({ store: { isMultiPush: false }, client: { request: async () => ({ data: { cyiUgNvO2EPltj: { atY3WWbXIN: 3000 }, pl9xRlV: 'Service unavailable' } }) } }); const result = await provider.loadHomeData({ loadCycleId: 'cycle-business', viewRevision: 0, trigger: 'initial' }); assert.equal(result.status, 'business_failure'); assert.equal(result.viewPayload.pageStatus, 'loading'); assert.equal(result.viewPayload.toastNotice.text, 'Service unavailable') })
