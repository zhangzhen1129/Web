...
const METHOD = 'openFaceCamera'
const ACTION = 'openFaceCamera'
const CALLBACK_SCOPE = 'perCall'
const CALLBACK_PREFIX = '__dineroProFaceCameraReply_'
...
function reportDiagnostic(code) {
  ...
  try {
    window.dispatchEvent(new CustomEvent('dinero-pro:bridge-diagnostic', {
      detail: { code, bridge: BRIDGE_OBJECT, capability: 'face-camera' },
    }))
  } catch {
...
function isValidReply(reply, requestId) {
  return reply !== null
    ...
    && Number.isInteger(reply.originalBytes)
    && Number.isInteger(reply.compressedBytes)
}

/** Open the documented Android face camera and deliver its terminal result. */
export function openFaceCameraNative(consumer = () => {}, options) {
  ...
  const registryKey = createId('face-camera')
  const requestId = createId('face-camera-request')
...
export function cancelNativeFaceCameraConsumer(requestId) {
...
export function getNativeFaceCameraRegistrySize() {
  return registry.size
}

export const nativeFaceCameraBridge = Object.freeze({
